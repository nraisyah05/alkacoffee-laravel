                        ...
                        <div class="col-md-2">
                            <select name="role" onchange="this.form.submit()" class="form-select">
                                ...
                                <option value="Super Administrator"
                                    {{ request('role') == 'Super Administrator' ? 'selected' : '' }}>Super
                ...
                <table class="table table-centered table-nowrap mb-0 rounded" id="table-user">
                    <thead class="thead-light">
                        <tr>
                            ...
                            <th class="border-0">Username</th>  {{-- TAMBAH INI --}}
                            <th class="border-0">Email</th>
                            <th class="border-0">Role</th>
                            <th class="border-0">Gambar User</th>
                    ...
                    <tbody>
                        @foreach ($dataUser as $row)
                            <tr>
                                ...
                                <td>{{ $row->name }}</td>
                                <td>{{ $row->username }}</td>  {{-- TAMBAH INI --}}
                                <td>{{ $row->email }}</td>
                                <td>{{ $row->role }}</td>
                                <td>
                                    @if ($row->profil)
                                        <img src="{{ asset('profil/' . $row->profil) }}" width="60px"
                                            class="rounded" alt="Gambar User">
                                    @else
                                ...
                                <td>
                                    ...
                                    <a href="{{ route('user.destroy', $row->id) }}" class="btn btn-danger btn-sm"
                                        onclick="return confirm('Yakin ingin menghapus user {{ $row->name }}?')">
                                        <svg class="icon icon-xs me-2" fill="none" stroke-width="1.5"
                                            stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                                            <path stroke-linecap="round" stroke-linejoin="round"
