...
@section('content')
    ...
    <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <div class="d-block mb-4 mb-md-0">
            ...
            <h2 class="h4">Data Asset</h2>
            <p class="mb-0">List data asset</p>
        </div>
    ...
    <div class="card border-0 shadow mb-4">
        <div class="card-body">
            {{-- filter --}}
            <div class="mb-3">
                <form method="GET" action="{{ route('asset.list') }}">
                    <div class="row">
                        {{-- filter kategori --}}
                        <div class="col-md-2">
                            <select name="kategori" onchange="this.form.submit()" class="form-select">
                                <option value="">Semua Kategori</option>
                                <option value="Mesin" {{ request('kategori') == 'Mesin' ? 'selected' : '' }}>Mesin</option>
            ...
            <div class="table-responsive">
                <table class="table table-centered table-nowrap mb-0 rounded" id="table-asset">
                    <thead class="thead-light">
                        <tr>
                            <th class="border-0 rounded-start">#</th>
                            <th class="border-0">Nama Asset</th>
                    ...
                    <tbody>
                        @foreach ($dataAsset as $row)
                            <tr>
                                <td> {{($dataAsset->currentPage() - 1) * $dataAsset->perPage() + $loop->iteration}} </td>
                                ...
                                <td> <a href="{{ route('asset.edit', $row->asset_id) }}" class="btn btn-info btn-sm">
                                    ...
                                    <a href="{{ route('asset.destroy', $row->asset_id) }}" class="btn btn-danger btn-sm">
                                        <svg class="icon icon-xs me-2" data-slot="icon" fill="none" stroke-width="1.5"
                                            stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"
                                            aria-hidden="true">
                                            <path stroke-linecap="round" stroke-linejoin="round"
            ...
            {{-- pagination --}}
            <div class="mt-4">
                {{ $dataAsset->links('pagination::bootstrap-5') }}
            </div>
        </div>
    </div>
    {{-- end main content --}}
